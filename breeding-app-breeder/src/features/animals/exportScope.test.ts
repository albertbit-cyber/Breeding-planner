...
describe('what counts as for sale', () => {
  it('reads the flag, the status and the tags', () => {
    expect(isAnimalForSale({ forSale: true })).toBe(true);
    expect(isAnimalForSale({ status: 'For Sale' })).toBe(true);
    expect(isAnimalForSale({ tags: ['available'] })).toBe(true);
  });
