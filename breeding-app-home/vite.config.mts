  ...
  return {
    ...
    configureServer(server) {
      server.httpServer?.once("listening", () => {
        const port = (server.config.server.port as number) ?? 5178;
...
// Absolute base, not "./": this app is served through a history-fallback router,
// so a deep URL like /pricing must still resolve /assets/... from the site root.
