...
// breeding-app-shared is an ES module package: no require / __dirname.
const HERE = path.dirname(fileURLToPath(import.meta.url));

const MATERIALS_CSS = process.env.MATERIALS_CSS
  || path.join(HERE, '..', 'materials.css');
const SKINS_CSS = process.env.SKINS_CSS
  || path.join(HERE, '..', 'skins.css');
 ...
 *
 *  NOTE: no material here is `hc: 'ok'`. The only two that were (soapstone,
 *  basalt) have been removed, so `high-contrast-forest` now offers `flat`
 *  alone — `flat` has no [data-material] block by design and so never appears
 *  in this table.
 *
 *  ok      — ships
