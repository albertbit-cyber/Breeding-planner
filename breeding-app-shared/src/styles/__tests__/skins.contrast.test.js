...
// breeding-app-shared is an ES module package, so no __dirname / require.
const HERE = path.dirname(fileURLToPath(import.meta.url));
const CSS_PATH = process.env.SKINS_CSS || path.join(HERE, '..', 'skins.css');
...
export { run, ratio, parseHex, parseSkins };
...
// ── vitest / jest ───────────────────────────────────────────────────────────
const UNDER_TEST = typeof process !== 'undefined' && process.env.VITEST !== undefined;
if (UNDER_TEST) {
