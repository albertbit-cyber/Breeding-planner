...
$LocalJdkRoot = Join-Path $RepoRoot ".tools\jdk21"
$FallbackJdkRoot = Join-Path $RepoRoot ".tools\jdk17"
$LocalAndroidSdk = Join-Path $RepoRoot ".tools\android-sdk"
...
if (!(Test-Path $AndroidDir)) {
  throw "Android project not found at $AndroidDir. Run 'npx cap add android' from breeding-app-lab first."
}
...
if ($BuildType -eq "release") {
  $keystore = $env:BREEDING_PLANNER_LAB_KEYSTORE_FILE
  $storePassword = $env:BREEDING_PLANNER_LAB_KEYSTORE_PASSWORD
  $alias = $env:BREEDING_PLANNER_LAB_KEY_ALIAS
  $keyPassword = $env:BREEDING_PLANNER_LAB_KEY_PASSWORD
  ...
  if ((!$keystore -or !$storePassword -or !$alias -or !$keyPassword) -and !(Test-Path $keyProperties)) {
    ...
    Write-Warning "Set BREEDING_PLANNER_LAB_KEYSTORE_FILE, BREEDING_PLANNER_LAB_KEYSTORE_PASSWORD, BREEDING_PLANNER_LAB_KEY_ALIAS, and BREEDING_PLANNER_LAB_KEY_PASSWORD, or create android/key.properties."
  }
}
