...
export const createLabApiClient = () => {
  ...
  const listLabAvailableTests = async (): Promise<LabAvailableTest[]> => {
    requireSessionRole("admin", "lab_staff");
    // The acting laboratory's own tests, not the platform's shared library. The
    // backend resolves which laboratory from the caller's membership, so there
    // is no id here that could point at somebody else's catalogue.
  ...
  const createLabAvailableTest = async (input: CreateLabAvailableTestInput): Promise<LabAvailableTest> => {
    ...
    if (input.catalogRefId) payload.catalogRefId = String(input.catalogRefId).trim();
    const response = await apiRequest<{ offering: any }>("/lab/my/tests", {
      method: "POST",
