...
**Two onboarding paths, not one.** **Decided 2026-07-30: one organization per user, as a hard rule everywhere** — every `User` belongs to exactly one `Organization`, auto-created at breeder signup (the "personal organization" pattern). This keeps today's single-user experience unchanged for existing accounts while giving every account an org to grow into multi-seat later without a second migration. Multi-org membership is explicitly out of scope, including for lab staff: a technician working across two vendor labs needs two accounts. Implement accordingly — there is deliberately no "active org" / org-switcher concept to thread through permission checks or UI. This is the one decision in this plan that is genuinely expensive to reverse, so if it ever needs revisiting, treat it as its own migration project rather than an incremental change.
...
**Resolved 2026-07-30 (was an open question blocking 4.1):** vendor-lab organizations are **not billed**. Lab Portal access is free for admin-vetted service partners; platform revenue comes from breeder-side subscriptions and/or a per-order fee on lab work instead. Concretely, for everything in this phase: `Organization.kind = lab_vendor` orgs skip the checkout/paywall path entirely and keep whatever access the admin granted at invite time; `SubscriptionTier`/`UserSubscription`/`UsageTracking` apply to **breeder orgs only**; and seat limits (4.4) don't apply to lab orgs either. A per-order fee on lab work is the intended lab-side revenue mechanism but is **not** scoped in this plan — it's a separate piece of work to spec if and when it's pursued.
...
2. Remaining Phase 0 work, as of 2026-07-30 — **neither item blocks Phase 1**:
   - **0.5 (cookie auth)** is built but deliberately inert. Activating it needs a decision that doesn't exist yet: there are currently *two* parallel live deployments per app — Railway services and separate Netlify sites — and no decision on which is the permanent host. Building a Railway-side reverse proxy (which Railway needs and Netlify already solves for free) before that's settled risks wasted work. Leave inert until the hosting question is answered deliberately.
   - **0.6 (Sentry)** is code-complete and verified against the live deployed bundle (correct DSN, `environment`, and `release` all present at the real `Sentry.init()` call site on `serpentora.com`). **Still unverified:** whether session/event data is actually *arriving* in serpentora.sentry.io — needs a dashboard look, not more code.
3. **Both blocking product decisions are now settled (2026-07-30):**
   - **Breeder org membership — one org per user, as a hard rule everywhere.** Every `User` belongs to exactly one `Organization`, auto-created at breeder signup (the "personal organization" pattern in 3.1). No multi-org membership anywhere, including for lab staff: a technician working across two vendor labs needs two accounts. This deliberately trades that edge case away for a materially simpler schema and permission model — there is no "active org" concept to thread through every permission check and UI surface. **Note the asymmetry to respect when implementing:** this is the one decision here that is genuinely expensive to reverse later, since adding multi-org membership after the fact means revisiting every permission check written against it.
   - **Lab vendor orgs are NOT billed.** Lab Portal access is free for admin-vetted service partners; revenue comes from breeder subscriptions and/or per-order fees on lab work instead. Concretely, this resolves the open question at the top of Phase 2: `Organization.kind = lab_vendor` orgs skip the checkout/paywall path entirely and keep whatever access the admin granted at invite time, and 4.1's payment-gateway work applies to breeder orgs only. Seat limits (4.4) likewise don't apply to lab orgs.
...
5. Still to decide, but *during* 3.1/3.4 design rather than before it: the specific permission matrix per org role (`owner`/`admin`/`billing_manager`/`member`) against each existing action. Unlike the two decisions above, this one is cheap to revise — it's authorization logic, not schema shape.
