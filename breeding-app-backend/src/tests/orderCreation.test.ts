...
vi.mock("../services/labConfigService", () => ({
  listCatalog: vi.fn(async () => []),
  getActivePricing: vi.fn(async () => ({ id: "pricing-1" })),
}));
...
describe("createOrder", () => {
  ...
  it("never hands the backfill a transaction client", async () => {
    ...
    // Called once here and once by the getOrderByIdForUser read that follows; never with a client.
  ...
  it("asks only for this month's highest order number", async () => {
    await createOrder("breeder-1", []);

    const [args] = db.shedTestOrder.findMany.mock.calls[0];
    // Reading every order number in the database is what made the cost of placing an order
    // grow with the number of orders ever placed.
  ...
  it("keeps the per-test price split the loop produced", async () => {
    ...
    expect(testArgs.data).toEqual([
      ...
      { orderAnimalId: "oa-1", testId: "t-morph-1", testNameSnapshot: "Clown", pricingTypeSnapshot: "morph", priceApplied: 60 },
      { orderAnimalId: "oa-1", testId: "t-morph-2", testNameSnapshot: "Piebald", pricingTypeSnapshot: "morph", priceApplied: 30 },
      { orderAnimalId: "oa-1", testId: "t-sex", testNameSnapshot: "Sexing", pricingTypeSnapshot: "sex", priceApplied: 20 },
      { orderAnimalId: "oa-2", testId: "t-morph-3", testNameSnapshot: "Albino", pricingTypeSnapshot: "morph", priceApplied: 60 },
    ]);
  });
